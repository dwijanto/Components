...
Public Class TEUCPController
    Private myform As FormTEUCP
    ...
    Public Sub New(ByVal obj As FormTEUCP)
        Me.myform = obj
    End Sub
    ...
    Private Sub AssignForwarderId()
        ...
        Dim Sqlstr = "select * from wkteunofixcomp"
    ...
    Sub DoWork()
        ...
        sqlstr = String.Format("select *,'cietd' as status,getloadinggroup(loadingcode) as ""loadingcode(grouped)""  from vteuicomp where currentinquiryetd >= '{0:yyyy-MM-dd}' and currentinquiryetd <= '{1:yyyy-MM-dd}'" &
             " Union All " &
             " select *,'cietd_past' as status,getloadinggroup(loadingcode) as ""loadingcode(grouped)""  from vteuicomp where currentinquiryetd < '{0:yyyy-MM-dd}' " &
             " Union All " &
             " select *,'ccetd' as status,getloadinggroup(loadingcode) as ""loadingcode(grouped)"" from  vteucomp where ccetd >= '{0:yyyy-MM-dd}' and ccetd <= '{1:yyyy-MM-dd}'" &
             " Union All" &
             " select *,'ccetd_past' as status,getloadinggroup(loadingcode) as ""loadingcode(grouped)"" from vteucomp where ccetd <= '{0:yyyy-MM-dd}'", myform.StartDate, myform.EndDate, myform.ForecastStart)
            ...
            Select Case id
                ...
                Case 4
                    ...
                    If (DirectoryBrowser.ShowDialog() = Windows.Forms.DialogResult.OK) Then
                        ...
                        Dim reportname = "TEUCP" '& GetCompanyName()

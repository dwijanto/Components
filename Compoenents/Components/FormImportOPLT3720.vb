...
Public Class FormImportOPLT3720
    ...
    Sub DoWork()
        ...
        If DbAdapter1.getproglock("FImOPLTSIS", HelperClass1.UserInfo.DisplayName, 1) Then
        ...
        Else
            ...
            If ReadFileStatus Then
                sw.Stop()
                DbAdapter1.getproglock("FImOPLTSIS", HelperClass1.UserInfo.DisplayName, 0)
    ...
    Private Function ImportTextFile(ByVal FileName As String, ByRef errMsg As String) As Boolean
        ...
        Try
            ...
            DbAdapter1.deleteOPLTSIS(startdate)
            ...
            Dim OPLT As New OPLTNew3720(DS)
    ...
    Private Function FillDataset(ByRef DS As DataSet, ByRef errmessage As String) As Boolean
        Dim myret As Boolean = False
        Dim sb As New StringBuilder
        'sb.Append(String.Format("select pd.* from purchdocsis pd" &
        '                        " left join  purchdocdtlsis dt on dt.purchdoc = pd.purchdoc " &
        '                        " left join opltdtlsis d on d.purchdocdtlid = dt.purchdocid " &
        '                        " left join oplthdsis h on h.salesdoc = d.salesdoc" &
        '                        " where h.sdhddocdate >= '{0:yyyy-MM-dd}';", startdate))
        'sb.Append(String.Format("select dt.* from purchdocdtlsis dt " &
        '                        " left join opltdtlsis d on d.purchdocdtlid = dt.purchdocid" &
        '                        " left join oplthdsis h on h.salesdoc = d.salesdoc " &
        '                        " where h.sdhddocdate >='{0:yyyy-MM-dd}';", startdate))
        'sb.Append(String.Format("select hd.* from oplthdsis hd where hd.sdhddocdate >= '{0:yyyy-MM-dd}';", startdate))
        'sb.Append(String.Format("select odt.* from opltdtlsis odt left join oplthd hd on hd.salesdoc = odt.salesdoc" &
        '                       " where hd.sdhddocdate >='{0:yyyy-MM-dd}';", startdate))
        sb.Append(String.Format("select pd.purchdoc from purchdocsis pd;")) 'POHD
        sb.Append(String.Format("select dt.purchdocid,dt.purchdoc,purchdocitem from purchdocdtlsis dt;")) 'PODTL
        sb.Append(String.Format("select hd.salesdoc from oplthdsis hd; ")) 'OPLTHD
        sb.Append(String.Format("select odt.purchdocdtlid,odt.salesdoc,odt.item from opltdtlsis odt;")) 'OPLTDTL
        sb.Append(" select opltdtlid from opltdtlsis  order by opltdtlsis desc limit 1;select purchdocid  from purchdocdtlsis order by purchdocid desc limit 1 ;")



        If DbAdapter1.TbgetDataSet(sb.ToString, DS, errmessage) Then
            DS.Tables(0).TableName = "purchdocsis"
            DS.Tables(1).TableName = "purchdocdtlsis"
            DS.Tables(2).TableName = "oplthdsis"
            DS.Tables(3).TableName = "opltdtlsis"
            ...
            idx1(0) = DS.Tables(1).Columns(1) '1
            idx1(1) = DS.Tables(1).Columns(2) '2
...
Public Class OPLTNew3720
    ...
    Public Function buildSB(ByRef message As String, ByVal mydata As myData) As Boolean
        ...
        Try
            ...
            If IsNothing(result) Then
                ...
                podtlid = seqpodtl
                dr.Item(0) = podtlid
                dr.Item(1) = data(11) '11
                dr.Item(2) = data(12) '12
    ...
    Public Function copyToDb(ByRef errMsg As String, ByVal myform As FormImportOPLT3720) As Boolean
        Dim myret As Boolean = False
        ...
        Try
            ...
            If cxopltsaleshd.ToString <> "" Then
                ...
                sqlstr = "copy oplthdsis(salesdoc,ponumber,sdhddocdate,soldtoparty,shiptoparty) from stdin with null as 'Null';"
            ...
            If cxopltpohd.ToString <> "" Then
                ...
                sqlstr = "copy purchdocsis(purchdoc,vendorcode,pddocdate) from stdin with null as 'Null';"
            ...
            If cxopltpodtl.ToString <> "" Then
                ...
                sqlstr = "copy purchdocdtlsis(purchdoc,purchdocitem,createdon,leadtime,material,quantity,oun,ms) from stdin with null as 'Null';"
            ...
            If cxopltsalesdtl.ToString <> "" Then
                ...
                sqlstr = "copy opltdtlsis(salesdoc,item,createdby,rj,purchdocdtlid) from stdin with null as 'Null';"
